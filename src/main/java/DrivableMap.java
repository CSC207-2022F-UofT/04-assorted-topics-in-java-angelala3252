...
class DrivableMap {
    ...
    public boolean addDrivable(String id, Drivable obj) {
        if (this.drivable_map.putIfAbsent(id, obj) == null) {
            return true;
        }
        return false;
    }
    ...
    public boolean hasFasterThan(int speed) {
        for (Drivable driv : this.drivable_map.values()) {
            if (driv.getMaxSpeed() >= speed) {
                return true;
            }
        }
        return false;
    ...
    public List<Tradable> getTradable() {
        ...
        for (Drivable driv : this.drivable_map.values()) {
            if (driv instanceof Tradable) {
                result.add((Tradable) driv);
            }
        }
        return result;
    }
